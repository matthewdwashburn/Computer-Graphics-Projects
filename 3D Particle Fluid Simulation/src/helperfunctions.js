...
export class mat4 {
    ...
    //return inverted matrix using gauss-jordan elimination
    inverse() {
        let a = this.m;
        let result = new mat4();
        ...
        for (let i = 0; i < 4; i++) {
            ...
            if (Math.abs(aug[i][i]) < 1e-10) {
                throw new Error("Matrix is singular, so it cannot be inverted");
            }
            //Scale pivot row
            let pivot = aug[i][i];
            for (let j = 0; j < 8; j++) {
...
export function translate(x, y = 0, z = 0) {
...
export function scalem(x, y = 1, z = 1) {
    if (Array.isArray(x) && x.length == 3) {
        z = x[2];
        y = x[1];
        x = x[0];
