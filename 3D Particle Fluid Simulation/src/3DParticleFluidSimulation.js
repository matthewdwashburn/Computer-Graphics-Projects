...
let gl; // the actual webgl rendering context, allows communication to the gpu
let canvas; // the html canvas element that's being drawn on
let program; // compiled shader program (vertex + fragment shaders)
...
let vPosition; // store the index for the vPosition attribute in the shader
let vInstancePosition; // Store the index for the instanced position attribute in the shader
let particlePositions; //32-bit float array for sending instanced positions to the vertex shader
let instanceBuffer; //Create a personal buffer for sending instanced particle positions to the GPU
let densityBuffer; //Create a buffer to send densities over to the GPU for each particle
let useInstancing; //Bool in vertex shader that determines which position calculation to use
...
let fbBackground; //Background frame buffer
let texBackground; //Texture for the background
let rbBackground; //Need a render buffer to store depth info, we are drawing depth on the same renderbuffer
...
let fbDepth; //Depth frame buffer
let texDepth; //Texture for the depth value saved in alpha channel
...
let uBlurCompositeSampler;
//Blur uniforms
let uBlurDirection;
let uBlurInverseScreenSize;
let uBlurFilterRadius;
let uBlurSpatialScale;
let uBlurDepthFalloff;
...
let programComposite;
//Normal shader program that creates a normal map on the particles from the blurred composite depth map texture
let programNormal;
let fbNormal;
let texNormal;
let uNormalBlurSampler;
let uNormalInverseScreenSize;
let uNormalInverseProj;
let uNormalInverseView;
//Fluid lighting shader programs
let programFluidLighting;
let uFluidLightingNormalSampler;
let fbFluidLighting;
let texFluidLighting;
let uFluidLightingInverseScreenSize;
let uFluidLightingView;
let uFluidLightingInverseProj;
...
import { initFileShaders, vec4, mat4, flatten, perspective, translate, lookAt, rotateX, rotateY, rotateZ, } from './helperfunctions.js';
//Grab bounding box radius sliders and values from html
let boundingBoxSizeSlider = document.getElementById("boundingBoxSize");
let boundingBoxSizeValue = document.getElementById("boundingBoxSizeValue");
...
let restDensitySlider = document.getElementById("restDensity");
let restDensityValue = document.getElementById("restDensityValue");
...
let relaxationParameterSlider = document.getElementById("relaxationParameter");
let relaxationParameterValue = document.getElementById("relaxationParameterValue");
...
// filterRadius
let filterRadius;
let filterRadiusSlider = document.getElementById("filterRadius");
let filterRadiusValue = document.getElementById("filterRadiusValue");
...
// spatialScale
let spatialScale;
let spatialScaleSlider = document.getElementById("spatialScale");
let spatialScaleValue = document.getElementById("spatialScaleValue");
...
// depthFalloff
let depthFalloff;
let depthFalloffSlider = document.getElementById("depthFalloff");
let depthFalloffValue = document.getElementById("depthFalloffValue");
...
class RenderObject {
    ...
    render(gl, umv, parentMV = null, skipDraw = false) {
        let mv;
        ...
        let scaleMatrix;
        scaleMatrix = new mat4(new vec4(this.scaleFactor, 0, 0, 0), new vec4(0, this.scaleFactor, 0, 0), new vec4(0, 0, this.scaleFactor, 0), new vec4(0, 0, 0, 1));
...
function collisionDetection(particles) {
...
function carCollisionDetection(particles, car) {
    //Box dimensions
    let boxWidth = 4;
    let boxHeight = 4;
    let boxLength = 6;
...
function wheelCollisionDetection(particles, car) {
...
function fluidSimulation(particles) {
    ...
    for (let i = 0; i < nextParticles.length; i++) {
        let neighborList = []; // Reset for each particle
    ...
    for (let i = 0; i < solverIterations; i++) {
        ...
        for (let i = 0; i < nextParticles.length; i++) {
            ...
            for (let j = 0; j < allNeighborLists[i].length; j++) {
                if (i === j)
                    continue; //Skip self in gradient calculation
                let p1 = nextParticles[i];
                let p2 = nextParticles[allNeighborLists[i][j]];
                let dx = p1.xOffset - p2.xOffset;
                let dy = p1.yOffset - p2.yOffset;
...
function smoothKernelOptimized(r2) {
...
function spikyKernelGradientOptimized(dx, dy, dz, r) {
...
function calculateArtificialPressureOptimized(r2) {
...
function enforceBarriers(particles) {
    for (let i = 0; i < particles.length; i++) {
...
function getGridSpace(pX, pY, pZ) {
...
class Particle {
    constructor() {
        this.xOffset = 0;
        this.yOffset = particleInitialHeight;
        this.zOffset = 0;
        this.velocity = new vec4(0, 0, 0, 0);
...
class Sphere extends RenderObject {
    ...
    makeObjectVertices() {
        let step = (360.0 / circleSlices) * (Math.PI / 180.0);
        let sphereverts = [];
...
function initFluidRenderingPipeline(gl) {
...
function calculateLights(mv) {
    ...
    function setLightPosition(lightXOffset, lightYOffset, lightZOffset) {
...
function setLightUniforms(gl, targetProgram) {
...
function drawParticlesDepth(mv, p) {
...
function drawParticlesOriginal(mv, p) {
...
function render() {
    ...
    let pM = perspective(fov, canvas.clientWidth / canvas.clientHeight, 1.0, 1000.0);
    //Calculate view matrix (world space to eye space)
    let vM = lookAt(new vec4(cameraXOffset, cameraYOffset, cameraZOffset, 1), new vec4(cameraLookingAtXOffset, cameraLookingAtYOffset, cameraLookingAtZOffset, 1), (new vec4(0, 1, 0, 0)));
    ...
    if (renderMode == 0) {
        ...
        if (renderMode >= 1) {
            ...
            function renderModeThree() {
                ...
                let inverseProjection = pM.inverse(); //We need to take objects from clip space back into view space
                let inverseView = vM.inverse();
