import { initFileShaders, mat4, ortho, flatten, vec2 } from "./helperfunctions.js";
"use strict";
...
let umode; //what type of curve are we following?
let shader_t; //what is the free parameter value?
let controlPoints; //what is the current set of control points?
let mode;
let trace;
let bufferId;
let vPosition; //remember the location of shader attributes
let vColor; //remember the location of shader attributes
const NUM_POINTS = 24; //how many control points should there be? (for Bezier curves, try to make this a multiple of 3)
let p0 = 0; //Control point index 0 is the starting P0
let t = 0; //t is the free parameter
let tstep = 0.01; //delta t
let mv;
let verts;
...
function update() {
    ...
    if (t >= 1) { //Move to next set of control points
        ...
        let points = [verts[p0], verts[(p0 + 1) % NUM_POINTS], verts[(p0 + 2) % NUM_POINTS], verts[(p0 + 3) % NUM_POINTS]];
...
function makePointsBuffer() {
    ...
    let points = [verts[0], verts[1], verts[2], verts[3]];
    gl.uniform2fv(controlPoints, flatten(points));
}
//draw a new frame
function render() {
...
//# sourceMappingURL=curvesfunctions.js.map