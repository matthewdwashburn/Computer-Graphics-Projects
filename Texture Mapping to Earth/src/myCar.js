...
let canvas = document.getElementById("gl-canvas"); // the html canvas element that's being drawn on
//grab the WebGL 2 context for that canvas
let gl = canvas.getContext('webgl2'); // the actual webgl rendering context, allows communication to the gpu
//Earth Shader
//Take the vertex and fragment shaders provided and compile them into a shader program
let program = initFileShaders(gl, "vshader-combined.glsl", "fshader-combined.glsl"); // compiled earth shader program (vertex + fragment shaders)
//Cloud Shader
let cloudProgram = initFileShaders(gl, "vshader-cloud.glsl", "fshader-cloud.glsl"); // compiled cloud shader program (vertex + fragment shaders)
//Cloud shader model view and projection matrix
let cloudsUmv; //index of model_view in shader program
let cloudsUproj; //index of projection in shader program
...
let cloudLight_position;
let cloudLight_color;
let cloudLight_direction;
let cloudSpotLight_angle;
let cloudAmbient_light;
...
let textureMapOn;
let isTextureMap = true;
let specularMapOn;
let isSpecularMap = true;
let nightMapOn;
let isNightMap = true;
let normalMapOn;
let isNormalMap = true;
//Import helper functions
import { initFileShaders, vec4, vec2, mat4, flatten, perspective, translate, lookAt, rotateX, rotateY, rotateZ, } from './helperfunctions.js';
...
class RenderObject {
    ...
    render(gl, umv, uproj, sphereNumber) {
        let mv;
...
function setShaderLocations(currentProgram) {
...
function render() {
    ...
    let p = perspective(fov, canvas.clientWidth / canvas.clientHeight, 1.0, 1000.0);
    //Set view matrix for light calculations
    let mv = lookAt(new vec4(cameraXOffset, cameraYOffset, cameraZOffset, 1), new vec4(cameraLookingAtXOffset, cameraLookingAtYOffset, cameraLookingAtZOffset, 1), (new vec4(0, 1, 0, 0)));
